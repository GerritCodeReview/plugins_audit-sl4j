...
public class AuditHttpLogTest {
    ...
    @Test
    public void getCorrectUser_GIT_AccessPath() {
        AuditHTTPLog auditHTTPLog = new AuditHTTPLog("anyIP", "anyUser", "anyTimestamp", "amnyResource", "anyProtocol", "-",200, "-", "-","\"git/1.8.3.1\"");
        AuditUser auditUser = auditHTTPLog.getAuditUser();
        assertEquals("anyUser", auditUser.username);
        assertEquals(AccessPath.GIT, auditUser.getAccessPath());
    }

    @Test
    public void getCorrectUser_REST_API_AccessPath() {
    AuditHTTPLog auditHTTPLog =
        ...
        AuditUser auditUser = auditHTTPLog.getAuditUser();
        assertEquals("anyUser", auditUser.username);
        assertEquals(AccessPath.REST_API, auditUser.getAccessPath());
    }

    @Test
    public void getCorrectAccesPathREST_API() {
        assertNull(AuditHTTPLog.parseHTTPLogs("invalid http log"));
    }
